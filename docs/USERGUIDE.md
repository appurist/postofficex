...
PostOfficeX reads `./data/local.json` at startup by default. It also expects sibling `defaults.json` and `users.json` files in the same directory. If `./data/local.json` is missing, it falls back to `/etc/postofficex/local.json`. You can also pass `--config /path/to/local.json`.
...
```powershell
.\postofficex.exe --config "C:\mail\data\local.json"
```
...
- `hostname`: Optional SMTP-only hostname override.
  - If omitted, PostOfficeX uses the global `hostname` from `data/local.json`.
...
- `greetingHostname`: Optional outbound `EHLO` override.
  - If omitted, PostOfficeX uses `server.smtp.hostname`, which itself falls back to `data/local.json.hostname`.
...
- `data/local.json` at `/opt/postofficex/data/local.json`
- `certFile` set to `./certs/server.crt`
- actual certificate path becomes `/opt/postofficex/data/certs/server.crt`
...
If you keep `tls.certFile` and `tls.keyFile` pointed at `./certs/server.crt` and `./certs/server.key`, you can use the included Certbot scripts to populate `data/certs/` with Let’s Encrypt certificates.
...
The Certbot deploy hook copies the renewed certificate and key into `data/certs/` and signals the process with `SIGHUP`. PostOfficeX then reloads the TLS files from disk for:
...
- built-in config lookup from `./data/local.json`
...
- Set `data/local.json.hostname` to your real mail hostname.
